import unittest
from task_manager import TodoList, TaskStatus
...
class TestCompleteTodoList(unittest.TestCase):
    ...
    def test_mark_in_progress_if_already_in_progress_raises_error(self):
        self.todo_list.add_task("Tarefa inicial", "")
        task_id = list(self.todo_list.tasks.keys())[0]
        with self.assertRaises(ValueError):
            self.todo_list.mark_as_in_progress(task_id)
